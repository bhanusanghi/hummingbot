...
class DEMASTADXTokenStrategy(StrategyV2Base):
    ...
    def __init__(self, connectors: Dict[str, ConnectorBase], config: DEMASTADXTokenConfig):
        ...
        # Store closing indicators for executors
        self.executor_closing_indicators = {}
    ...
    def create_actions_proposal(self) -> List[CreateExecutorAction]:
        ...
        for i, trading_pair in enumerate(self.config.trading_pairs):
            ...
            if signal is not None and signal != 0:  # Only process non-zero signals
                ...
                # Capture indicator snapshot for position open
                open_indicators = self._create_indicator_snapshot(candles_pair, mid_price)
        ...
        for i, trading_pair in enumerate(self.config.trading_pairs):
            ...
            if current_supertrend_direction is not None:
                ...
                if current_supertrend_direction == -1 and len(active_longs) > 0:
                    # Capture closing indicators
                    close_indicators = self._create_indicator_snapshot(candles_pair)
                    for e in active_longs:
                        # Store close indicators for this executor
                        self.executor_closing_indicators[e.id] = close_indicators
            ...
            if adx_value < self.config.adx_threshold_choppy:
                ...
                for executor in all_positions:
                    ...
                    if executor.net_pnl_pct > 0.002:  # 0.2% profit threshold
                        # Capture closing indicators
                        close_indicators = self._create_indicator_snapshot(candles_pair)
                        self.executor_closing_indicators[executor.id] = close_indicators
                        stop_actions.append(StopExecutorAction(
    ...
    def format_status(self) -> str:
        ...
        if active_executors:
            ...
            # Table header
            header = f"  {'Symbol':<12} {'Side':<5} {'P&L USD':<12} {'P&L %':<8} {'Size USD':<10} {'Fees':<8} {'Status':<10} {'Open-MC':<10}"
            separator = f"  {'-' * 12} {'-' * 5} {'-' * 12} {'-' * 8} {'-' * 10} {'-' * 8} {'-' * 10} {'-' * 10}"
            lines.extend([header, separator])
            ...
            for executor_info in active_executors:
                ...
                # Get open market condition from executor custom_info
                open_mc = executor_info.custom_info.get('open_indicators', {}).get('mc', 'N/A') if executor_info.custom_info else 'N/A'

                # Color coding for P&L (you can add ANSI colors if supported)
                row = f"  {symbol:<12} {side:<5} {pnl_usd:<12} {pnl_pct:<8} {size:<10} {fees:<8} {status:<10} {open_mc:<10}"
                lines.append(row)
        else:
    ...
    def _create_indicator_snapshot(self, candles_pair: str, price: Optional[float] = None) -> Dict[str, Any]:
        """
        Create a snapshot of current market indicators for persistence.
        """
        return {
            'timestamp': self.current_timestamp,
            'price': float(price) if price else float(self.current_price.get(candles_pair, 0)),
            'dema': float(self.current_dema.get(candles_pair, 0)),
            'supertrend_direction': int(self.current_supertrend_direction.get(candles_pair, 0)),
            'adx': float(self.current_adx.get(candles_pair, 0)),
            'mc': self.market_condition.get(candles_pair, 'UNKNOWN'),
            'plus_di': float(self.current_plus_di.get(candles_pair, 0)),
            'minus_di': float(self.current_minus_di.get(candles_pair, 0)),
            'adx_slope': float(self.adx_slope.get(candles_pair, 0))
        }
